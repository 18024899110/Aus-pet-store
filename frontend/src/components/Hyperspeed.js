  ...
  useEffect(() => {
    ...
    const initializeApp = (container) => {
      ...
      if (rect.width > 0 && rect.height > 0) {
        ...
        console.warn('Hyperspeed: Container size is zero, retrying...');
      }
    };

    // 使用多次重试机制确保容器有尺寸后再初始化
    let retryCount = 0;
    const maxRetries = 10;
    const retryDelay = 50; // 50ms 间隔
    ...
    const tryInit = () => {
      ...
      if (!container) {
        if (retryCount < maxRetries) {
          retryCount++;
          currentTimer = setTimeout(tryInit, retryDelay);
        }
        return;
      }
      ...
      if (rect.width > 0 && rect.height > 0) {
        ...
        console.error('Hyperspeed: Failed to initialize after maximum retries');
      }
    };

    // 开始初始化尝试
    currentTimer = setTimeout(tryInit, 0);
